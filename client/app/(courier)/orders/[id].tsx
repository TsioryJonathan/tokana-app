...
export default function CourierOrderDetail() {
  ...
  const [history, setHistory] = useState<{ id: number; from?: OrderStatus | null; to: OrderStatus; at: string }[]>([]);
  ...
  const updateStatus = async (next: 'en_route_vers_recuperation' | 'en_chemin' | 'en_chemin_pour_livraison' | 'expedie') => {
    if (!id) return;
    setSubmitting(true);
    try {
  ...
  const canToEnChemin = order.status === 'PICKED_UP' || order.status === 'CREATED';
  const canToOutForDelivery = order.status === 'IN_TRANSIT' || order.status === 'PICKED_UP';
  ...
  return (
    <View className="flex-1 bg-slate-50">
      <View className="px-4 py-3 flex-row items-center bg-white border-b border-slate-200">
        ...
        <Text className="ml-4 text-lg font-quicksand-bold text-slate-900 flex-1">Course #{order.code}</Text>
      </View>

      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <View className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
          <Text className="text-slate-500 text-sm">Statut actuel</Text>
          <Text className="mt-1 text-base font-quicksand-bold text-emerald-600">{statusLabel[order.status]}</Text>
          <Text className="mt-2 text-slate-600 text-sm">Type: {order.service === 'EXPRESS' ? 'Express' : 'Standard'}</Text>
          {order.service === 'EXPRESS' && eta && (
            <Text className="text-[12px] text-emerald-700 mt-1">ETA: {eta.min}–{eta.max} min</Text>
          )}
          {typeof order.priceAr === 'number' ? (
            <Text className="text-slate-600 text-sm">Prix: {order.priceAr.toLocaleString()} Ar</Text>
          ) : null}
        ...
        <View className="mt-4 bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
          <Text className="text-base font-quicksand-bold text-slate-900">Actions</Text>
          <View className="mt-3 gap-8">
            <View>
              <Text className="text-slate-700 font-quicksand-semibold">Transitions</Text>
              <View className="mt-2 flex-row flex-wrap gap-8">
                <TouchableOpacity disabled={!canToPickup || submitting} onPress={() => updateStatus('en_route_vers_recuperation')}>
                  <Text className={`${canToPickup && !submitting ? 'text-emerald-700' : 'text-slate-400'} font-quicksand-semibold`}>Aller récupérer</Text>
                </TouchableOpacity>
                <TouchableOpacity disabled={!canToEnChemin || submitting} onPress={() => updateStatus('en_chemin')}>
                  <Text className={`${canToEnChemin && !submitting ? 'text-emerald-700' : 'text-slate-400'} font-quicksand-semibold`}>En chemin</Text>
                </TouchableOpacity>
        ...
        <View className="mt-4 bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
          <Text className="text-base font-quicksand-bold text-slate-900">Remarques</Text>
          {loadingRemarks ? (
            ...
            <View className="mt-2">
              {remarks.length === 0 ? (
                ...
                remarks.map((r, idx) => (
                  <View key={`${r.id}-${idx}`} className="py-2 border-b border-slate-100">
                    <Text className="text-[13px] text-slate-800">{r.text}</Text>
                    <Text className="text-[11px] text-slate-400">{new Date(r.createdAt).toLocaleString()}</Text>
                  </View>
                ))
              )}
            </View>
          ...
          <View className="mt-3">
            ...
            <View className="mt-2 flex-row justify-end">
              <TouchableOpacity disabled={submitting || newRemark.trim().length < 2} onPress={addRemark}>
                <Text className={`${!submitting && newRemark.trim().length >= 2 ? 'text-emerald-700' : 'text-slate-400'} font-quicksand-semibold`}>Envoyer</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View className="mt-4 bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
          <Text className="text-base font-quicksand-bold text-slate-900">Historique</Text>
          {history.length === 0 ? (
            <Text className="mt-2 text-slate-500 text-sm">Aucun historique disponible.</Text>
          ) : (
            <View className="mt-2">
              {history.map((h, idx) => (
                <View key={`${h.id}-${idx}`} className="flex-row items-start py-2 border-b border-slate-100">
                  <View className="mt-1 mr-3"><Ionicons name="time-outline" size={14} color="#64748B" /></View>
                  <View className="flex-1">
                    <Text className="font-quicksand-semibold text-slate-800">{statusLabel[h.to]}</Text>
                    <Text className="text-[12px] text-slate-500">{new Date(h.at).toLocaleString()}</Text>
                  </View>
                </View>
              ))}
            </View>
