...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = useCallback((message: string, type: ToastType = "info") => {
    ...
    try {
      ...
      import("expo-haptics").then((H) => {
        ...
        if (type === "success") return H.notificationAsync(H.NotificationFeedbackType.Success);
        if (type === "error") return H.notificationAsync(H.NotificationFeedbackType.Error);
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      {/* Toast container */}
      <View pointerEvents="none" style={{ position: "absolute", left: 0, right: 0, top: 10 }}>
        {toasts.map((t) => (
...
function ToastItem({ message, type }: { message: string; type: ToastType }) {
  ...
  const bg = type === "error" ? "#FEE2E2" : type === "success" ? "#DCFCE7" : "#E2E8F0";
  const border = type === "error" ? "#FCA5A5" : type === "success" ? "#86EFAC" : "#CBD5E1";
  const color = type === "error" ? "#7F1D1D" : type === "success" ? "#065F46" : "#0F172A";

  return (
    <Animated.View style={{ opacity }}>
      <View style={{ marginHorizontal: 12, marginBottom: 8, paddingVertical: 10, paddingHorizontal: 12, borderRadius: 12, backgroundColor: bg, borderWidth: 1, borderColor: border, alignSelf: "center", maxWidth: "92%" }}>
        <Text style={{ color, fontWeight: "600" }}>{message}</Text>
      </View>
    </Animated.View>
  );
}
