...
export default {
  up: async (queryInterface) => {
    ...
    try {
      // ✅ Chemin robuste Windows/macOS/Linux
      const dataPath = path.join(__dirname, "..", "data", "zones.json");

      // (facultatif) petit check utile en debug
      // console.log("Reading zones from:", dataPath);
      ...
      for (const [key, zone] of Object.entries(zones)) {
        ...
        let existing = await queryInterface.sequelize.query(
          ...
          {
            replacements: { key: normalizedKey },
        ...
        if (!zoneId) {
          const inserted = await queryInterface.sequelize.query(
            'INSERT INTO "Zones" (key, label, "createdAt", "updatedAt") VALUES (:key, :label, :createdAt, :updatedAt) RETURNING id',
            {
              replacements: {
                key: normalizedKey,
                label: zone.label || normalizedKey,
                createdAt: now,
                updatedAt: now,
              },
          ...
          zoneId =
            Array.isArray(inserted) && inserted[0]?.[0]?.id
              ? inserted[0][0].id
              : undefined;
          if (!zoneId) {
      ...
      for (const [zoneKeyRaw, zone] of Object.entries(zones)) {
        ...
        for (const [axisKey, localities] of Object.entries(axes)) {
          ...
          let ares = await queryInterface.sequelize.query(
            ...
            {
              replacements: { zoneId, key: axisKey },
              type: queryInterface.sequelize.QueryTypes.SELECT,
              transaction,
            }
          );
          ...
          if (!axisId) {
            ...
            axisId =
              Array.isArray(insertedAxis) && insertedAxis[0]?.[0]?.id
                ? insertedAxis[0][0].id
                : undefined;
            if (!axisId) {
