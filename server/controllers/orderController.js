...
const statusSchema = Joi.object({
  status: Joi.string().valid('en_cours_de_traitement', 'en_route_vers_recuperation', 'en_chemin', 'en_chemin_pour_livraison', 'expedie').required(),
});
...
export const createOrder = async (req, res, next) => {
  try {
    ...
    // Slots validation
    if (type === 'standard') {
      if (!isStandardOrderWindow(new Date())) return res.status(400).json({ msg: 'Créneau de commande standard fermé' });
      if (!slotStart || !slotEnd) return res.status(400).json({ msg: 'slotStart et slotEnd requis pour une commande standard' });
      const slots = getStandardSlots(zoneLevel);
      const ok = slots.some(s => s.startISO === new Date(slotStart).toISOString() && s.endISO === new Date(slotEnd).toISOString());
      if (!ok) return res.status(400).json({ msg: 'Créneau invalide pour la zone' });
    } else {
      const { allowed } = getExpressAvailability(new Date());
      if (!allowed) return res.status(400).json({ msg: 'Commande express non disponible pour le moment' });
    }
...
export const requestDeliveryOtp = async (req, res, next) => {
  try {
    ...
    if (order.status !== 'en_chemin_pour_livraison') {
      return res.status(400).json({ msg: "OTP autorisé uniquement lorsque la commande est en chemin pour livraison" });
    }
    ...
    const lastAt = order.deliveryOtpLastRequestedAt ? new Date(order.deliveryOtpLastRequestedAt).getTime() : 0;
    ...
    if (lastAt && now < nextAllowedAt) {
      ...
      return res.status(429).json({ msg: 'Trop de demandes OTP, réessayez plus tard', retryAfter });
    }
    ...
    const hash = crypto.createHash('sha256').update(code + secret).digest('hex');
    ...
    if (channel === 'sms') {
      if (!destPhone) return res.status(400).json({ msg: 'Numéro destinataire manquant' });
      if (!mgPhone.test(destPhone)) return res.status(400).json({ msg: 'Téléphone MG invalide (ex: +261201234567 ou 0201234567)' });
      await sendSms(destPhone, msg);
    } else {
      if (!destEmail) return res.status(400).json({ msg: 'Email destinataire manquant' });
...
export const verifyDeliveryOtp = async (req, res, next) => {
  try {
    ...
    const secret = process.env.OTP_SECRET || '';
    const hash = crypto.createHash('sha256').update(value.code + secret).digest('hex');
    ...
    return res.json({ msg: 'OTP vérifié', deliveryOtpVerifiedAt: order.deliveryOtpVerifiedAt });
  } catch (err) {
    next(err);
  }
};
  ...
  try {
    ...
    if (role === 'livreur') {
      if (!userId || order.assignedTo !== userId) {
        return res.status(403).json({ msg: 'Non autorisé à modifier cette commande' });
      }
    }
    ...
    if (!allowedNext.includes(to)) {
      return res.status(400).json({ msg: `Transition interdite: ${from} → ${to}` });
    }

    // Enforce OTP verification before final delivery
    if (to === 'expedie') {
      if (!order.deliveryOtpVerifiedAt) {
        return res.status(400).json({ msg: 'OTP de preuve requis avant expédition' });
      }
    }
...
export const listOrders = async (req, res, next) => {
  try {
    ...
    if (role !== 'admin') {
      ...
      // Livreur convenience filter
      if (role === 'livreur' && assignedTo === 'me') {
