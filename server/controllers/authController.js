...
import { generateAccessToken, createRefreshToken, findTokenRecordByRaw, rotateRefreshToken, revokeToken, revokeAllUserTokens } from '../services/tokenService.js';
...
const registerSchema = Joi.alternatives().try(
  Joi.object({
    ...
    phone: Joi.string().pattern(/^(\+261|0)(3[0-9]|20)\d{7}$/).required().messages({
      'string.pattern.base': 'Téléphone MG invalide (ex: +261201234567 ou 0201234567)',
...
export const register = async (req, res, next) => {
  try {
    ...
    if (phone) {
      ...
      if (existingPhone) return res.status(400).json({ msg: 'Téléphone déjà utilisé' });
    }

    // Always force default role at registration (client-side role is ignored)
    user = await User.create({ email: email ?? null, password, role: 'client', name, phone });

    const token = generateAccessToken({ id: user.id, role: user.role });
    const { raw: refreshToken } = await createRefreshToken(user.id, { userAgent: req.headers['user-agent'], ip: req.ip });

    res.status(201).json({ token, refreshToken, user: { id: user.id, email: user.email, role: user.role, name: user.name, phone: user.phone } });
  } catch (err) {
    next(err);
  }
};
...
export const login = async (req, res, next) => {
  try {
    ...
    res.json({ token, refreshToken, user: { id: user.id, email: user.email, role: user.role, name: user.name, phone: user.phone } });
  } catch (err) {
    next(err);
  }
};
...
export const refresh = async (req, res, next) => {
  try {
    ...
    if (record.rotatedAt) {
      ...
      return res.status(401).json({ msg: 'Refresh token réutilisé (compromis): toutes les sessions ont été révoquées' });
    }

    // rotate refresh token
    const { raw: newRefresh } = await rotateRefreshToken(record, { userAgent: req.headers['user-agent'], ip: req.ip });
